...
/** 矩形定位 */
type IRects = [number, number, number, number ];
interface IRect {
  x: number,
  y: number,
  width: number,
...
* 表格展示值，支持多样式字符串
*/
type ICellM = undefined | null | string | number | {t: string, c?: string, r?: boolean}[];
...
interface IFormula {
  name:string,
  calc: (r: number, c: number, proxy: {grid: ICell[][], getCell: (r:number, c:number) => ICell }, paramsStr:string) => Partial<ICell>;
  recount: (range: IRange, cell:ICell, paramsStr:string) => boolean;
}
